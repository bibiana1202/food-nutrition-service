...
`food_name`, `maker_name`(부분 일치)은 와일드카드를 리터럴로 처리하려고 `LIKE` 대신 `LOCATE(term, column) > 0`을 쓴다(설계 근거는 [3. 검색 API](#3-검색-api) 참고). 인덱스는 컬럼의 원래 값을 정렬해둔 것이라, `LOCATE(...)` 처럼 값을 함수에 넣어 계산한 결과로 조건을 걸면 그 정렬 순서를 활용할 수 없다 — 결국 매 행마다 `LOCATE`를 직접 계산해서 확인해야 한다. `ORDER BY id`가 있어서 PRIMARY(= id 순서)를 따라 행을 훑으며(정렬을 별도로 안 해도 되니까) 하나씩 이 계산을 반복하는 것이다.
...
지금 규모에서는 정확 일치(0.4ms)와 부분 일치(8.1ms) 모두 체감상 문제없는 속도라, 인덱스 없이 `LOCATE`로 처리하는 현재 방식이 합리적이다.

근본적으로 B-Tree 인덱스는 컬럼의 원래 값을 정렬해둔 구조라, `LOCATE`처럼 값을 함수에 넣어 계산한 결과로 조건을 거는 경우에는 그 정렬을 전혀 활용할 수 없다. FULLTEXT는 이 문제를 풀려고 만들어진 B-Tree가 아닌 다른 구조(역인덱스)라 이론적으로는 해결책이 될 수 있지만, 실제 한글 데이터로 검증해보니 복합어(예: "김치"로 검색했을 때 "김치찌개"를 못 찾는 경우)에서 정확도가 깨지는 것을 실측으로 확인했다. 즉 FULLTEXT로 바꾸면 속도는 얻지만 검색 결과가 틀려지는 대가를 치러야 한다.
...
`express-rate-limit`은 `app.set('trust proxy', N)`으로 Nginx 뒤에서도 실제 클라이언트 IP를 기준으로 카운트하도록 설정했다. 요청 제한을 초과하면 다른 오류와 동일한 형식의 응답(`RATE_LIMIT_EXCEEDED`)을 반환한다.

`trust proxy`를 `1`로 고정해뒀던 것이 실제 배포 구조와 안 맞는 버그였다. 로컬은 프런트엔드 컨테이너 Nginx 1단계만 거치지만, 운영은 [5. 배포](#5-배포)에 적었듯 호스트 Nginx(HTTPS 종료)와 프런트엔드 컨테이너 Nginx까지 **2단계**를 거친다. `trust proxy: 1`인 상태로 2단계를 거치면 Express가 실제 클라이언트 IP가 아니라 중간의 호스트 Nginx IP를 클라이언트로 오인하고, 호스트 Nginx는 항상 같은 IP이므로 **모든 사용자의 요청이 하나의 요청 제한 버킷을 공유**하게 된다. `NODE_ENV`로 환경을 구분해 로컬은 1단계, 운영은 2단계를 신뢰하도록 고쳤다(신뢰 단계를 실제보다 크게 잡으면 반대로 클라이언트가 `X-Forwarded-For`를 조작해 요청 제한을 우회할 수 있어, 더 크게 잡는 것도 정답은 아니다).
...
요청 제한이 IP 하나를 기준으로 카운트하므로, 같은 공인 IP를 여러 사용자가 공유하는 경우(같은 와이파이를 쓰는 사무실·카페, 통신사 NAT 등)에는 그 사용자들이 실제로는 서로 다른데도 하나의 요청 제한을 나눠 쓰게 된다. 이건 IP 기반 요청 제한의 근본적인 한계라 `trust proxy` 설정만으로는 해결이 안 되고, 정말 사용자 단위로 나누려면 로그인·API 키처럼 IP가 아닌 다른 식별자가 필요하다. 지금은 관리자 쓰기 작업만 키로 보호돼 있고 읽기는 익명 접근이라, 당장은 이 한계를 받아들이고 있다.
