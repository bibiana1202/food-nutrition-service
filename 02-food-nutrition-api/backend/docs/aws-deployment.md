...
3. 저장소를 내려받고 `02-food-nutrition-api`에서 `.env.example`을 `.env`로 복사한다. 충분히 긴 새 관리자 키를 설정한다.
4. `docker compose build`, `docker compose run --rm seed`, `docker compose up -d`를 순서대로 실행한다. `docker compose exec api npm run db:verify`로 데이터를 확인한다.
